"""Fetch monthly revenue from FinMind in chunks, saving progress incrementally.
...
from stock_llm.data.finmind import fetch_monthly_revenue
from stock_llm.data.store import connect, upsert_monthly_revenue
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--resume",
        action="store_true",
        help="Skip stocks already present in monthly_revenue table",
    )
    parser.add_argument(
        "--min-months",
        type=int,
        default=12,
        help="In --resume mode, only skip stocks that already have >= N months",
    )
    return parser.parse_args()


def _done_stocks(min_months: int) -> set[str]:
    with connect() as con:
        df = con.execute(
            """
...
def main() -> None:
    ...
    if args.resume:
        done = _done_stocks(args.min_months)
    ...
    for idx, i in enumerate(range(0, len(codes), args.chunk_size), start=1):
        ...
        df = fetch_monthly_revenue(chunk, start.isoformat(), end.isoformat(), sleep=args.sleep)
        if not df.empty:
            upsert_monthly_revenue(df)
